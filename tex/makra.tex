...
\newcommand{\abstrEN}{This diploma thesis analyses the causes of shrinkage and proposes a~procedure for evaluating products in terms of their susceptibility to shrinkage and depreciation. Based on the analysis, a general procedure to reduce shrinkage is proposed, applicable to data from other companies. Basic terms from logistics, types of waste, definition of shrinkage and its classification are discussed in the theoretical part of the thesis. The practical part includes data sample selection and preparation as well as creation of an interactive report in MS Power BI. Analyses performed allow to formulate and test hypotheses using the 4ftMiner procedure. Using correlation analysis, the products are categorized based on the shrinkage behaviour. This categorization is implemented in Python. Through this process, companies can evaluate their product portfolio and reduce their product write-offs and minimize the resulting lost costs.} % zde NAPIŠTE abstrakt v angličtině

\newcommand{\prohlaseni}{Prohlašuji, že jsem svou diplomovou práci vypracoval\woman{} samostatně a použil\woman{} jsem pouze podklady (literaturu, projekty, SW atd.) uvedené v přiloženém seznamu.} % text prohlášení můžete mírně upravit :-)
